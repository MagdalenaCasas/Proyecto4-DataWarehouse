...
const login = async()=>{
    ...
    try {
      ...
      fetch("http://localhost:3000/login", requestOptions)
        ...
        .then(result => { localStorage.setItem("esAdmin", `${result.admin}`);localStorage.setItem("id", `${result.id}`) ;localStorage.setItem("token", `Bearer ${result.token}`,location.href = 'contactos.html');})
